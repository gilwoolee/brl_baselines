...
class BDDPG(object):
    def __init__(self, actor, critic, experts, obs_dim, memory, observation_shape, action_shape,
        ...
        expert_qv0 = tf.squeeze(tf.stack([tf.stop_gradient(experts[i].critic(expert0_normalize_obs0[i], self.actions))\
        ...
        self.normalized_critic_tf = critic(normalized_obs0, self.actions, expert_qv0)
        self.critic_tf = tf.clip_by_value(self.normalized_critic_tf, self.return_range[0], self.return_range[1])

        expert_qv0_with_actor_tf = tf.squeeze(tf.stack([tf.stop_gradient(experts[i].critic(expert0_normalize_obs0[i], self.actor_tf)) for i in range(len(self.experts))]),
            axis=2)
        expert_qv0_with_actor_tf = tf.reduce_sum(self.obs0[:, self.obs_dim:] * tf.transpose(expert_qv0_with_actor_tf), axis=1)

        self.normalized_critic_with_actor_tf = critic(normalized_obs0, self.actor_tf, expert_qv0_with_actor_tf)
        ...
        expert0_normalize_obs1 = [tf.clip_by_value(normalize(self.obs1[:, :self.obs_dim], self.experts[i].obs_rms),
            self.observation_range[0], self.observation_range[1]) for i in range(len(self.experts))]
        expert_qv1 = tf.squeeze(tf.stack([tf.stop_gradient(experts[i].critic(expert0_normalize_obs1[i], action1)) for i in range(len(self.experts))]), axis=2)
        expert_qv1 = tf.reduce_sum(self.obs1[:, self.obs_dim:] * tf.transpose(expert_qv1), axis=1)

        self.Q_obs1 = target_critic(normalized_obs1, action1, expert_qv1)
    ...
    def train(self):
        ...
        #         self.critic_obs0: batch['obs0'][:, self.obs_dim]
        #         self.critic_obs1: batch['obs1'][:, self.obs_dim]
        ...
        target_Q, Q_obs1, expert_qv1 = self.sess.run([self.target_Q, self.Q_obs1, self.expert_qv1], feed_dict={
            self.obs1: batch['obs1'],
            self.rewards: batch['rewards'],
            self.terminals1: batch['terminals1'].astype('float32'),
        })
