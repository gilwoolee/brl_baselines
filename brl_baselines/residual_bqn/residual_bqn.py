...
from brl_baselines import residual_bqn
from brl_baselines.residual_bqn.build_graph import build_act
from brl_baselines.residual_bqn.models import build_q_func
from brl_baselines.residual_bqn.replay_buffer import ReplayBufferWithExperts as ReplayBuffer
...
def learn(env,
          ...
          residual_learning_starts=100000,
          gamma=1.0,
    ...
    **network_kwargs
    ...
    act, train, expert_train, update_target, debug = residual_bqn.build_train(
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        while t < total_timesteps:
            ...
            for m in range(horizon):
                ...
                #     # write to file
                #     # with open('tiger_rbqn.csv', 'a') as f:
            ...
            if t > learning_starts: # and t % train_freq == 0:
                ...
                # Minimize the error in Bellman's equation on a batch sampled from replay buffer.
                    ...
                    # td_errors, expert_td_errors = train(obses_t, actions, rewards, obses_tp1, dones, weights, expert_i)


                    if np.random.rand() < 0.05:
                        print("Expert TD error",np.mean(expert_td_errors), np.mean(td_errors))

                    if prioritized_replay:
                        new_priorities = np.abs(expert_td_errors) + np.abs(td_errors) + prioritized_replay_eps
            #             replay_buffer.update_priorities(batch_idxes, new_priorities)

            # if t > residual_learning_starts:
            #     experience = replay_buffer.sample(batch_size, expert=None)
            #     if experience is None:
            #         continue
            #     obses_t, bels_t, actions, rewards, obses_tp1, bels_tp1, dones, exps = experience
            #     weights, batch_idxes = np.ones_like(rewards), None

            #     # expert_td_errors = expert_train(obses_t, bels_t, actions, rewards, obses_tp1, bels_tp1, dones, weights, expert_i)

            #     if np.random.rand() < 0.05:
            #         print("TD error", np.mean(td_errors))
