    ...
    def q_func_builder(input_placeholder, belief_placeholder, num_actions, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            with tf.variable_scope("bel", reuse=reuse):
                ...
                log_denominator = tf.log(tf.constant(num_actions, dtype=belief_placeholder.dtype))
                entropy = -1 * tf.reduce_sum(belief_placeholder * tf.log(belief_placeholder + 1e-5) / log_denominator, axis=1)
